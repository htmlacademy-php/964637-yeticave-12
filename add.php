...
if (isset($_FILES['lot-img'])) {
    if ($_FILES['lot-img']['size'] === $emptyFile) {
        $addLotErr['lot-img'] = 'Изображение не выбрано';
        ...
        if (!move_uploaded_file($_FILES['lot-img']['tmp_name'], $file_path . $file_name)) {
            $addLotErr['lot-img'] = 'Ошибка загрузки файла';
        }
    }
}
...
if (isset($_POST['submit']) && count($addLotErr) === $emptyError) {
    ...
    $getAddLotId = getLastLot($conn);

    if ($addLot) {
        header('Location: lot.php?id=' . (int) $getAddLotId[0]['MAX(id)']);
        exit;
    }
}
